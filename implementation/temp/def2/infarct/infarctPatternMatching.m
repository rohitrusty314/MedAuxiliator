...
I = imread('E:\CBIR\implementation\temp\def\1085.jpg'); 
I = rgb2gray(I);
...
c=1;
 ...
 c = c + 1;       
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         for l=1:size(ek)
             count = 0;
             for k = 1:size(ekinfarct)
                 if(ekinfarct(k,1) == ek(l,1))
                     count = count + 1;
                     for id = 2:size(ekinfarct)
                         e = energy((ek(l,2)+ekinfarct(id,2)),(ek(l,3)+ekinfarct(id,3)),psi_rk);
                         if(ekinfarct(id,1) == e)
                             count = count + 1;
                         else
                             break;
        ...
        r=25;
        th=0:0.1:360;
        total = size(ekinfarct);
        totalr = total(1);
         
        x = 1;
        for h=1:size(mat)
            if( ((mat(h,1)/totalr) > 0.50)  & ((mat(h,1)/totalr) < 0.55) )
                (mat(h,1)/totalr)
                res(x,1) = mat(h,2);
                res(x,2) = mat(h,3);
                x = x + 1;
        ...
        load normalInfarctMatchIndices;
        ...
        for t = 1:size(res)
            for u = 1:size(normalInfarctMatch)
%                 if((abs(res(t,1)-normalInfarctMatch(u,1))<66) & (abs(res(t,2)-normalInfarctMatch(u,2))<66))
%                     res(t,3) = 0;
%                     break;
%                 end
            end
