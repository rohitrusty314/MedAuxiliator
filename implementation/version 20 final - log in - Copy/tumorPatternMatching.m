function [] = tumorPatternMatching(I,cid)
...
c=1;
 ...
 c = c + 1;       
  ...
  %calculate energy values for 3x3 windows     
         ...
         load bleedEnergy;          %load energy values of tumor
         count = 0;
         v = 1;
             ...
             for k = 1:size(ekbleed)
                 if(ekbleed(k,1) == ek(l,1))
                     ...
                     for id = 2:size(ekbleed)
                         e = energy((ek(l,2)+ekbleed(id,2)),(ek(l,3)+ekbleed(id,3)),psi_rk);  %calculate energy for the current window
        ...
        %Indices on match with normal Tumor Energy pattern
